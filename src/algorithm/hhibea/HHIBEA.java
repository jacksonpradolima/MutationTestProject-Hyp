 ...
 */
public class HHIBEA extends HyperHeuristic {

	private static final long serialVersionUID = 3272843013615524826L;
	...
	double calcHypervolumeIndicator(Solution p_ind_a, Solution p_ind_b, int d,
		...
		if (d == 1) {
			...
			if (a < b) {
				...
				volume = calcHypervolumeIndicator(p_ind_a, p_ind_b, d - 1,
						maximumValues, minimumValues) * (max - a) / r;
				// (max - b) / r;
			}
		}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (evaluations < maxEvaluations) {
			...
			while (offSpringSolutionSet.size() < populationSize) {
				...
				} while (j < HHIBEA.TOURNAMENTS_ROUNDS); // do-while
				int k = 0;
				do {
					k++;
					parents[1] = (Solution) selectionOperator.execute(archive);
				} while (k < HHIBEA.TOURNAMENTS_ROUNDS); // do-while
