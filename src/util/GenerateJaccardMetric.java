...
import statistics.JaccardCoefficient;
...
public class GenerateJaccardMetric {
    ...
    public static void generateJaccardCoefficient(String filename) {
        ...
        MTProblem p = new MTProblem("instances/" + filename);

        double sum = 0.0;
        double count = 0.0;

        double[][] result = new double[p.problemBase.getNumberOfTestSuite()][p.problemBase.getNumberOfTestSuite()];

        System.out.println("i\tj\tsimilarity");

        for (int i = 0; i < p.problemBase.getNumberOfTestSuite(); i++) {
            for (int j = i; j < p.problemBase.getNumberOfTestSuite(); j++) {
                if (i != j) {
                    List<String> one = new ArrayList<String>();
                    List<String> two = new ArrayList<String>();

                    for (int h = 0; h < p.problemBase.getNumberOfMutants(); h++) {
                        if (p.problemBase.isKilled(i, h)) {
                            one.add(String.valueOf(h));
                        }
                        if (p.problemBase.isKilled(j, h)) {
        ...
        System.out.println("Test Case=" + p.problemBase.getNumberOfTestSuite());
        System.out.println("Mutant=" + p.problemBase.getNumberOfMutants());
